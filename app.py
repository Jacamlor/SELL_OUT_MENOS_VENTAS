...
if uploaded_file:
    try:
        ...
        if df.shape[1] < 6:
            ...
            # Limpiar A1:A68 y toda F
            col_a = df.iloc[0:68, 0].fillna("").astype(str).str.strip()
            col_f = df.iloc[:, 5].fillna("").astype(str).str.strip()
            ...
            for valor in col_f:
                if valor in col_a_set:
                    ...
                    fila = col_a[col_a == valor].index[0] + 2  # Fila humana
                    fila_en_a.append(fila)
                else:
                    resultados.append("No")
                    fila_en_a.append("")

            df["Valor comprobado (col F)"] = df.iloc[:, 5].fillna("").astype(str).str.strip()
            ...
            st.download_button(
                ...
                data=excel_data,
                file_name="resultado_f_en_a_limpio.xlsx",
